...
(function initPresenceSystem() {
    ...
    function pollPresence() {
        ...
        Promise.all([
            ...
            fetch(API_BASE_URL + `/api/presence?escola=${escolaParam}`).then(r => r.json()).catch(() => [])
            ...
            allUsers.forEach(x => {
                if (!uniqueUsersMap.has(x.email) || x.avatarType === 'uploaded') {
                    uniqueUsersMap.set(x.email, x);
                }
            });
            const deduplicatedUsers = Array.from(uniqueUsersMap.values());

            // Filter deduplicatedUsers to only include those from the same school
            const schoolUsers = deduplicatedUsers.filter(usr => {
    ...
    window.renderDashboardRecentArticles = function() {
        ...
        try {
            ...
            container.innerHTML = recent.filter(r => r).map(r => {
                let fallback = 'assets/cat_tecidos.png';
                try {
                    fallback = typeof getCategoryFallbackImage === 'function' ? getCategoryFallbackImage(r.category) : fallback;
                } catch(e) {}
                
                const safeImg = r.image || fallback;
                
                let catLabel = r.category || 'Geral';
                try {
                    catLabel = typeof getCategoryLabel === 'function' ? getCategoryLabel(r.category) : catLabel;
                } catch(e) {}
            ...
            container.innerHTML = '<div style="color: red; padding: 20px;">ERRO JS: ' + error.message + '</div>';
            console.error(error);
        }
    };
