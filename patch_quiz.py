...
def patch_file(filepath):
    ...
    new_questions = """const COURSE_QUESTIONS = {
    module1: [
        {
    ...
    content = re.sub(r'const COURSE_QUESTIONS = \{.*?\n\};\n', new_questions + '\n', content, flags=re.DOTALL)

    # Update openQuizModal
    # Replace getLesson2Questions to handle module1 as well
    getLesson2Questions_new = """function getLesson2Questions(moduleId) {
    ...
    content = re.sub(r'function getLesson2Questions\(moduleId\) \{.*?return null;\n\}', getLesson2Questions_new, content, flags=re.DOTALL)

    openQuizModal_new = """function openQuizModal(moduleId) {
    ...
    content = re.sub(r'function openQuizModal\(moduleId\) \{.*?window\.openQuizModal = openQuizModal;', openQuizModal_new + '\nwindow.openQuizModal = openQuizModal;', content, flags=re.DOTALL)

    # Handle submit quiz (replace handleQuizExamSubmit)
    handleQuizExamSubmit_new = """function handleQuizExamSubmit(e) {
    e.preventDefault();
    ...
    if (lessonQuestions) {
        ...
        if (passed) {
            ...
            saveCourseProgress(progress);
            showToast(`❌ Reprovado. Você acertou ${correctCount} de ${totalQuestions}.`, 'error');
            setTimeout(() => {
                closeModal('modal-quiz-exam');
                renderCourseUI();
            }, 2500);
...
"""
    content = re.sub(r'function handleQuizExamSubmit\(e\) \{.*?\}\n\}\n(?=// ======================================================)', handleQuizExamSubmit_new, content, flags=re.DOTALL)
    
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)

patch_file('app_v2.js')
patch_file('app.js')
