...
function patch_file() {
    let content = fs.readFileSync('index.html', { encoding: 'utf-8' });

    // 1. Almoxarifado Tab & Pane in Coordenação
    const target_tab = '<button class="subtab-btn coord-subtab-btn" data-tab="almoxarifados"';
    if (!content.includes('data-tab="logistica"')) {
    ...
    const target_pane = '<!-- Pane: Gestão de Almoxarifados -->';
    const replacement_pane = `<!-- Pane: Inteligência Logística (Coord) -->
    ...
    if (!content.includes('coord-pane-logistica')) {
        content = content.replace(target_pane, replacement_pane);
    }

    // 2. Iframe Replacement
    if (content.includes('id="video-iframe"')) {
        content = content.replace(/<iframe id="video-iframe"[\s\S]*?<\/iframe>/m, '<video id="video-iframe" src="" width="100%" style="flex: 1; min-height: 350px; background: #000;" controls></video>');
    }
    
    // 3. Agenda Category Button
    const agenda_target = '<select id="calendar-category-filter"';
    if (!content.includes('id="btn-agenda-add-cat"')) {
        ...
        const modal_target = '<!-- DIALOG MODAL: AGENDAR EVENTO -->';
        const modal_replacement = `<!-- DIALOG MODAL: NOVA CATEGORIA AGENDA -->
