    ...
    if (lessonQuestions) {
        ...
        if (passed) {
            ...
            setTimeout(() => {
                ...
                else if (typeof showCertificateModal === 'function') showCertificateModal();
            }, 2000);
        } else {
            saveCourseProgress(progress);
            if (progress.attempts['exam'] >= maxAttempts) {
                ...
                showToast(`❌ Reprovado. Você acertou ${correctCount} de ${totalQuestions}. Mínimo: ${passingScore}.`, 'error');
                selectedAnswers = {};
                currentQuizQuestionIndex = 0;
                renderExamStep();
            }
...
window.renderCharts = function () {
    ...
    if (almoxContainer) {
        ...
        if (schoolLabs.length === 0) {
            almoxContainer.innerHTML = '<div style="color: var(--text-muted); padding: 20px; text-align: center;">Nenhum almoxarifado cadastrado nesta escola.</div>';
        } else {
            const counts = {};
            let total = 0;
...
function renderInsumosDestinoChart() {
    ...
    const colors = ['#556b2f', '#d4ac0d', '#3a8ee6', '#9b59b6', '#2ecc71', '#e67e22', '#16a085'];
    let colorIdx = 0;
    let currentPct = 0;
    let conicParts = [];
    let legendHtml = '';
...
function checkChatAndEstelaVisibility() {
    ...
    if (profChatBtn) {
        if (!localStorage.getItem('registeredUser') && !isCoord) {
            ...
            profChatBtn.style.display = 'flex';
            if (isCoord) {
                if (typeof window.updateCoordChatBadge === 'function') window.updateCoordChatBadge();
            } else {
                if (typeof window.updateProfChatBadge === 'function') window.updateProfChatBadge();
            }
        }
...
window.renderCoordChatDetailInProfWindow = function(boletimCode) {
    ...
    if (msgs.length === 0) {
        container.innerHTML = '<div style="color:var(--text-muted);text-align:center;padding:20px;font-size:0.9rem;">Nenhuma mensagem enviada. Comece a conversa!</div>';
    } else {
        msgs.forEach((m, idx) => {
            const div = document.createElement('div');
            const isMe = !m.isProf;
...
window.toggleRecording = async function(isProf) {
    ...
    const boletimCode = actualIsProf ? window.currentProfChatBoletimCode : window.currentCoordChatBoletimCode;
            ...
            window.mediaRecorder.onstop = function() {
                ...
                reader.onloadend = function() {
                    ...
                    const newMsg = {
                        id: Date.now() + Math.floor(Math.random() * 1000),
                        type: 'chat',
                        title: actualIsProf ? `Áudio do Professor (Boletim #${boletimCode})` : `Áudio da Coordenação (Boletim #${boletimCode})`,
                        audio: base64Audio,
                        time: new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }),
                        timestamp: new Date().toISOString(),
                        read: false,
                        ...
                        boletimCode: boletimCode,
                        isProf: actualIsProf,
                        senderName: actualIsProf ? (window.currentUser ? window.currentUser.name : 'Professor') : 'Coordenação'
                    };
...
(function initPresenceSystem() {
    ...
    function sendHeartbeat() {
        ...
        try {
            ...
            const presenceData = JSON.parse(localStorage.getItem('senaivest_presence') || '{}');
    ...
    function pollPresence() {
        ...
        try {
            ...
            if (regUser && regUser.email && !allUsers.find(x => x.email === regUser.email)) {
            ...
            const mappedUsers = schoolUsers.map(usr => {
                ...
                const isOnline = presEntry && (now - presEntry.lastSeen < ONLINE_THRESHOLD);
            ...
            if (self) { self.isOnline = true; ordered.push(self); }
            mappedUsers.forEach(usr => { if (usr.email !== u.email && usr.isOnline) ordered.push(usr); });
            mappedUsers.forEach(usr => { if (usr.email !== u.email && !usr.isOnline) ordered.push(usr); });
