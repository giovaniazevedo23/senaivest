...
window.openProfChatInbox = function() {
    ...
    let msgs = (window.notifications || []).filter(n => n.type === 'chat' || n.title.includes('Chat com Coordena��o') || n.title.includes('Plano de Aula Exclu�do') || n.title.includes('Observa��o da Coordena��o'));
    
    if (msgs.length === 0) {
        container.innerHTML = '<div style="color:var(--text-muted);text-align:center;padding:20px;">Nenhuma mensagem da coordena��o.</div>';
    } else {
        msgs.forEach(m => {
            m.read = true;
...
function updateProfChatBadge() {
    ...
    if (!badge) return;
    let msgs = (window.notifications || []).filter(n => (n.type === 'chat' || n.title.includes('Chat com Coordena��o') || n.title.includes('Plano de Aula Exclu�do') || n.title.includes('Observa��o da Coordena��o')) && !n.read);
    if (msgs.length > 0) {
...
window.saveCoordObsOnly = async function (boletimId) {
    ...
    if (observacao) {
        ...
        window.notifications.unshift({
            id: Date.now() + Math.floor(Math.random() * 1000),
            title: '?? Observa��o da Coordena��o (Boletim #' + b.id + ')',
            message: observacao,
            time: new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }),
            type: 'chat',
            read: false,
    ...
    syncWithBackend('boletins', registeredBoletins);
    if(typeof showToast==='function') showToast('Observa��o salva e enviada ao professor!', 'success');
};
...
window.promptStatusUpdate = async function(boletimId, newStatus) {
    ...
    if (b.createdBy && b.createdBy.includes('@')) {
        let msgToProfessor = 'O status do boletim de ocorr�ncia ' + b.code + ' (relacionado a: ' + b.titulo + ') mudou de ' + oldStatus + ' para ' + newStatus + '.';
        ...
        const content = '<h3>Atualiza��o de Boletim</h3><p>O status do boletim de ocorr�ncia <strong>' + b.code + '</strong> (' + b.titulo + ') mudou de <em>' + oldStatus + '</em> para <strong>' + newStatus + '</strong>.</p>';
        try {
            await fetch('https://api.resend.com/emails', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + window.RESEND_API_KEY },
                body: JSON.stringify({ from: 'SenaiVest <onboarding@resend.dev>', to: [b.createdBy], subject: '[SenaiVest] Atualiza��o do Boletim ' + b.code, html: content })
            });
