...
function replaceLines(filepath, blocks) {
    let lines = fs.readFileSync(filepath, 'utf8').split('\n');
    
    // Sort blocks in descending order by startLine to avoid shifting issues!
    blocks.sort((a, b) => b.startLine - a.startLine);

    for (const block of blocks) {
        const replacementLines = block.content.split('\n');
        lines.splice(block.startLine - 1, (block.endLine - block.startLine) + 1, ...replacementLines);
    }

    fs.writeFileSync(filepath, lines.join('\n'), 'utf8');
}

const COURSE_QUESTIONS_STR = `const COURSE_QUESTIONS = {
    module1: [
        {
...
const GET_LESSON_STR = `function getLesson2Questions(moduleId) {
...
const OPEN_QUIZ_STR = `function openQuizModal(moduleId) {
...
const HANDLE_SUBMIT_STR = `function handleQuizExamSubmit(e) {
    e.preventDefault();
...
replaceLines('app_v2.js', [
    { startLine: 8890, endLine: 9069, content: COURSE_QUESTIONS_STR },
    { startLine: 9803, endLine: 9808, content: GET_LESSON_STR },
    { startLine: 9850, endLine: 9906, content: OPEN_QUIZ_STR },
    { startLine: 9991, endLine: 10172, content: HANDLE_SUBMIT_STR }
]);

replaceLines('app.js', [
    { startLine: 9056, endLine: 9235, content: COURSE_QUESTIONS_STR },
    { startLine: 9969, endLine: 9974, content: GET_LESSON_STR },
    { startLine: 10016, endLine: 10072, content: OPEN_QUIZ_STR },
    { startLine: 10157, endLine: 10338, content: HANDLE_SUBMIT_STR }
]);

function replaceVideos(filepath) {
    let content = fs.readFileSync(filepath, 'utf8');
    content = content.split("'lesson2': 'assets/video.mp4',").join("'lesson2': 'assets/video IV.mp4',");
    content = content.split("'lesson3': 'assets/video.mp4'").join("'lesson3': 'assets/video III.mp4'");
    
    // For lessonsData
    content = content.split("duration: '12:45',\n            videoUrl: 'assets/video.mp4',").join("duration: '12:45',\n            videoUrl: 'assets/video IV.mp4',");
    content = content.split("duration: '12:45',\r\n            videoUrl: 'assets/video.mp4',").join("duration: '12:45',\r\n            videoUrl: 'assets/video IV.mp4',");
    
    content = content.split("duration: '14:20',\n            videoUrl: 'assets/video.mp4',").join("duration: '14:20',\n            videoUrl: 'assets/video III.mp4',");
    content = content.split("duration: '14:20',\r\n            videoUrl: 'assets/video.mp4',").join("duration: '14:20',\r\n            videoUrl: 'assets/video III.mp4',");
    
    fs.writeFileSync(filepath, content, 'utf8');
}
replaceVideos('app_v2.js');
replaceVideos('app.js');
